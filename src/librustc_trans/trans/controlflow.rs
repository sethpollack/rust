...
pub fn trans_if<'blk, 'tcx>(bcx: Block<'blk, 'tcx>,
    ...
    match els {
        Some(elexpr) => {
            ...
            CondBr(bcx, cond_val, then_bcx_in.llbb, else_bcx_in.llbb, NoSourceLoc);
        }

        None => {
            next_bcx = bcx.fcx.new_id_block("next-block", if_id);
            Br(then_bcx_out, next_bcx.llbb, NoSourceLoc);
            CondBr(bcx, cond_val, then_bcx_in.llbb, next_bcx.llbb, NoSourceLoc);
        }
    }
...
pub fn trans_while<'blk, 'tcx>(bcx: Block<'blk, 'tcx>,
    ...
    CondBr(cond_bcx_out, cond_val, body_bcx_in.llbb, cleanup_llbb, NoSourceLoc);
